...
module Codecamp::StaticAddthis
  ...
  def self.icons(options)
    providers = options[:providers].map{|p| link_to_provider(p, options) }.join(" ")

    if options[:inline]
      content_tag :span, providers, :class => "addthis_toolbox"
    else
      <<-HTML
  ...
  def self.link_to_provider(provider, options)
    ...
    when 'more'
      ...
      if known_provider?(provider)
        href = "//www.addthis.com/bookmark.php?pub=#{username}&amp;v=250&amp;source=tbx-250&amp;tt=0&amp;s=#{provider}&amp;url=#{url}&amp;title=#{title}"
